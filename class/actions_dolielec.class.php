...
if (!empty($parameters['currentcontext']) && $parameters['currentcontext'] === 'external') {
	if (is_object($object) && $object->element === 'ticket') {
		...
		if (preg_match('/presupuesto|instalaci[oó]n/', $titulo . ' ' . $mensaje)) {
															// Evitar múltiples respuestas automáticas
			if (empty($object->note_private) || strpos($object->note_private, 'BIEL_AUTO_RESPONDIDO') === false) {
				...
				$response .= "- Dirección completa (calle, número, piso…)\n";
				$response .= "- Ciudad, provincia\n";
				$response .= "- Teléfono o móvil\n";
				$response .= "- Correo electrónico\n";
				$response .= "- DNI, NIE o tarjeta de residencia\n";
				$response .= "- Y si eres particular, empresa, autónomo o comunidad de vecinos\n\n";
				$response .= "En cuanto tengamos esto, creamos la ficha y seguimos. Esto es necesario e imprescindible para darte una atención personalizada.";
